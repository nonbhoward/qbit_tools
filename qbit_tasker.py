...
class QbitTasker:
    def __init__(self):
        ...
        self.active_search_ids, self.search_header = dict(), ''
    ...
    def initiate_and_monitor_searches(self):
        ...
        try:
            for section_header in self.search_parser.sections():
                self.pause_between(SEARCHES)
                self.search_header = section_header
            ...
            try:
                with open(self.search_config_filename, 'w') as search_config_file:
                    ...
                    self.search_parser.write(search_config_file)
    ...
    def pause_between(self, pause_type):
        try:
            if pause_type == LOOPS:
    ...
    def _add_results_by_popularity(self, filtered_results) -> bool:
        ...
        """
        search_id = self.active_search_ids.get(self.search_header, '')
        if search_id == '':
        ...
        if most_popular_results is not None:
            for result in most_popular_results:
                ...
                minimum_seeds = int(self.search_parser[self.search_header]['minimum_seeds'])
            ...
            self.search_parser[self.search_header]['results_added'] = str(int(
                self.search_parser[self.search_header]['results_added']) + results_added)
    ...
    def _config_get_search_term(self) -> str:
        try:
            search_term = self.search_parser[self.search_header]['search_term']
            return search_term
        except KeyError as k_err:
            ml.log_event(k_err)
    ...
    def _config_increment_search_try_counter(self):
        try:
            search_try_count = int(self.search_parser[self.search_header]['search_attempts'])
            self.search_parser[self.search_header]['search_attempts'] = str(search_try_count + 1)
        except KeyError as k_err:
            ml.log_event(k_err)

    def _config_set_search_id_as_active(self, search_id):
        try:
            self.search_parser[self.search_header]['search_id'] = search_id
            self.active_search_ids[self.search_header] = search_id
        except KeyError as k_err:
            ml.log_event(k_err)

    def _config_set_search_id_as_inactive(self, search_id):
        try:
            if search_id == self.active_search_ids[self.search_header]:
                self.search_parser[self.search_header]['search_id'] = str(0)
                del self.active_search_ids[self.search_header]
        except KeyError as k_err:
            ml.log_event(k_err)
    ...
    def _filter_results(self):
        ...
        for result in results['results']:
            ...
            search_pattern = self.search_parser[self.search_header]['search_filter']
    ...
    def _get_active_search_ids(self) -> str:
        try:
            active_search_id = self.active_search_ids.get(self.search_header)
    ...
    def _get_search_state(self) -> tuple:
        ...
        """
        try:
            ml.log_event('get search state for section: {}'.format(self.search_header))
            self.search_parser[self.search_header]['last_read'] = str(datetime.datetime.now())
            _search_queued = self.search_parser[self.search_header].getboolean('search_queued')
            _search_running = self.search_parser[self.search_header].getboolean('search_running')
            _search_stopped = self.search_parser[self.search_header].getboolean('search_stopped')
            _search_concluded = self.search_parser[self.search_header].getboolean('search_concluded')
            ml.log_event('search state for {}: \nqueued: {}\nrunning: {}\nfinished: {}\nadded: {}'.format(
                self.search_header, _search_queued, _search_running, _search_stopped, _search_concluded))
    ...
    def _qbit_get_search_status(self) -> str:
        ...
        """
        try:
            ml.log_event('checking search status for section: {}'.format(self.search_header))
            search_id, search_status = self.search_parser[self.search_header]['search_id'], None
            if not search_id == '':
                ml.log_event('getting search status for section: {} with search_id: {}'.format(
                    self.search_header, search_id))
            ...
            ml.log_event('search status for section: {} is {}'.format(self.search_header, search_status))
            return search_status
        except ConnectionError as c_err:
            event = 'unable to process search for section: {}'.format(self.search_header)
            ml.log_event(event)
    ...
    def _reset_search_ids(self):
        ...
        try:
            for section_header in self.search_parser.sections():
                self.search_header = section_header
                ml.log_event('reset search_id for section: {}'.format(self.search_header))
                self._update_search_states(RESET)
        except KeyError as k_err:
            ml.log_event(k_err)
    ...
    def _search_yielded_required_results(self) -> bool:
        ...
        """
        try:
            attempted = int(self.search_parser[self.search_header]['search_attempts'])
            maximum_attempts = int(self.search_parser[self.search_header]['maximum_search_attempts'])
            added = int(self.search_parser[self.search_header]['results_added'])
            required = int(self.search_parser[self.search_header]['results_required'])
    ...
    def _update_search_states(self, job_state):
        try:
            if job_state == RESET or job_state == QUEUED:
                ...
                self.search_parser[self.search_header]['search_queued'] = YES
                self.search_parser[self.search_header]['search_running'] = NO
                self.search_parser[self.search_header]['search_stopped'] = NO
                self.search_parser[self.search_header]['search_concluded'] = NO
            elif job_state == STARTING:
                self.search_parser[self.search_header]['search_queued'] = NO
                self.search_parser[self.search_header]['search_running'] = YES
                self.search_parser[self.search_header]['search_stopped'] = NO
                self.search_parser[self.search_header]['search_concluded'] = NO
                ...
                self.search_parser[self.search_header]['search_queued'] = NO
                self.search_parser[self.search_header]['search_running'] = NO
                self.search_parser[self.search_header]['search_stopped'] = YES
                self.search_parser[self.search_header]['search_concluded'] = NO
            elif job_state == CONCLUDED:
                self.search_parser[self.search_header]['search_queued'] = NO
                self.search_parser[self.search_header]['search_running'] = NO
                self.search_parser[self.search_header]['search_stopped'] = NO
                self.search_parser[self.search_header]['search_concluded'] = YES
            else:
                pass
            self.search_parser[self.search_header]['last_write'] = str(datetime.datetime.now())
        except KeyError as k_err:
            ml.log_event(k_err)
