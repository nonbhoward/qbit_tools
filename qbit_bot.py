...
class QbitTasker:
    ...
    def initiate_and_monitor_searches(self):
        try:
            ...
            section_headers = self.parsers.search_details_parser.sections()
    ...
    def pause_on_event(self, pause_type):
        try:
            ...
            uc_keys = self.key_ring.parser_keyring
            if pause_type == uc_keys.MAIN_LOOP:
                delay = int(self.parsers.user_config_parser[uc_keys.DEFAULT][uc_keys.MAIN_LOOP])
                ml.log_event('{} waiting {} seconds for loop..'.format(timestamp, delay))
                sleep(delay)
            elif pause_type == uc_keys.SEARCH_STATUS_CHECK:
                delay = int(self.parsers.user_config_parser[uc_keys.DEFAULT][uc_keys.SEARCH_STATUS_CHECK])
                ml.log_event('{} waiting {} seconds for search..'.format(timestamp, delay))
                sleep(delay)
            elif pause_type == uc_keys.ADD_RESULT:
                delay = int(self.parsers.user_config_parser[uc_keys.DEFAULT][uc_keys.ADD_RESULT])
                ml.log_event('{} waiting {} seconds for add..'.format(timestamp, delay))
                sleep(delay)
            else:
                delay = int(self.parsers.user_config_parser[uc_keys.DEFAULT][uc_keys.USER])
                # TODO miscellaneous is hardcoded, not that it matters, just annoying and causes upkeep
                ml.log_event('{} waiting {} seconds miscellaneous..'.format(timestamp, delay))
                sleep(delay)
    ...
    def _all_searches_concluded(self) -> bool:
        ...
        try:
            as_keys, concluded = self.key_ring.api_state_keyring, list()
            # for section in self.config.parsers[SEARCH].sections():  # TODO delete
            for section in self.parsers.search_details_parser.sections():
                for key in section:
                    if key == as_keys.SEARCH_CONCLUDED:
                        # search_concluded = self.config.parsers[SEARCH][section].getboolean(key)  # TODO delete
                        search_concluded = self.parsers.search_details_parser[section].getboolean(key)
    ...
    def _check_if_search_is_concluded(self):
        try:
            ap_keys = self._get_api_states_keyring()
            if self._search_has_yielded_the_required_results():
                ...
                self._update_search_states(ap_keys.CONCLUDED)
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)
    ...
    def _parsers_get_file_name_regex(self) -> str:
        ...
        try:
            sd_keys = self.key_ring.search_detail_keyring
            search_details_parser_with_active_header = self._get_search_details_parser_at_active_header()
            file_name_regex = search_details_parser_with_active_header[sd_keys.FILE_NAME_REGEX]
            ...
            search_details_key_value = search_details_parser_with_active_header.keys()
            if file_name_regex not in search_details_parser_with_active_header.keys():
                file_name_regex = '.*'
                return file_name_regex
            # file_name_regex = self.config.parsers[SEARCH][self.active_header][FILENAME_REGEX]  # TODO delete
            file_name_regex = self.parsers.search_details_parser[self.active_header][file_name_regex]
            return file_name_regex
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)
    ...
    def _parsers_get_search_term(self) -> str:
        ...
        try:
            ...
            if search_term not in self.config.parser.parsers.search_details_parser[self.active_header].keys():
                ...
                return search_term
            search_term = self.config.parser.parsers.search_details_parser[self.active_header][search_term]
            return search_term
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)
    ...
    def _config_set_search_id_as_active(self):
        ...
        search_details_parser_at_active_header = self._get_search_details_parser_at_active_header()
        ...
        try:
            ...
            search_details_parser_at_active_header[s_keys.SEARCH_ID] = search_id
    ...
    def _get_api_states_keyring(self):
        try:
            api_states_keyring = self.config.hardcoded.keys.api_state_keyring
            return api_states_keyring
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)

    def _get_keyring_for_misc(self):
        try:
            misc_keyring = self.config.hardcoded.keys.misc_keyring
            return misc_keyring
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)

    def _get_parser_keyring(self):
        try:
            parser_keyring = self.config.hardcoded.keys.parser_keyring
            return parser_keyring
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)

    def _get_search_details_parser_at_active_header(self) -> ConfigParser:
        try:
            search_details_parser = self.parsers.search_details_parser[self.active_header]
            return search_details_parser
            pass
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)
    ...
    def _get_convenience_objects(user_configuration, debug=False) -> tuple:
        ...
        try:
            ...
            if debug:  # allow easy access to section_headers keys in search_detail_parser
                convenience_objects.append(user_configuration.parser.parsers.search_details_parser._sections)
            return * convenience_objects,
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)

    def _get_most_popular_results(self, filtered_results) -> list:
        # sd_keys, uc_keys = self.key_ring.search_detail_keyring, self.key_ring.user_config_keyring  # TODO delete
        sd_keys, uc_keys = self._get_keyring_for_search_details(), self._get_keyring_for_user_config()
        # expected_search_result_count = self.config.parsers[SEARCH][EXPECTED_RESULT_COUNT]
        expected_search_result_count = self.parsers.search_details_parser[self.active_header][sd_keys.RESULT_COUNT]
        ...
        try:
            ...
            ps_keys, uc_keys = self._get_parser_keyring(), self._get_keyring_for_user_config()
            user_config_parser_default_section = self.config.parser.parsers.user_config_parser[uc_keys.DEFAULT]
            ...
            _result_property_priority = uc_keys.PRIORITY
    ...
    def _get_priority_key_for_search_result_sorting(self):
        try:
            uc_keys = self.key_ring.user_config_keyring
            return self.config.parser.parsers.user_config_parser[uc_keys.DEFAULT][uc_keys.PRIORITY]
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)

    def _get_keyring_for_metadata_details(self):
        try:
            metadata_keyring = self.config.hardcoded.keys.metadata_details_keyring
    ...
    def _get_keyring_for_search_details(self):
        try:
            search_details_keyring = self.config.hardcoded.keys.search_detail_keyring
            return search_details_keyring
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)

    def _get_keyring_for_search_state(self):
        # TODO delete if deprecated after refactor
        try:
            search_state_keyring = self.config.hardcoded.keys.search_state_keyring
            return search_state_keyring
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)
    ...
    def _get_search_state(self) -> tuple:
        ...
        try:
            as_keys, sd_keys = self._get_api_states_keyring(), self._get_keyring_for_search_details()
            search_detail_parser_at_active_header = self._get_search_details_parser_at_active_header()
            search_detail_parser_at_active_header[sd_keys.LAST_READ] = str(datetime.now())
            _search_queued = search_detail_parser_at_active_header.getboolean(as_keys.QUEUED)
            _search_running = search_detail_parser_at_active_header.getboolean(as_keys.RUNNING)
            _search_stopped = search_detail_parser_at_active_header.getboolean(as_keys.STOPPED)
            _search_concluded = search_detail_parser_at_active_header.getboolean(as_keys.CONCLUDED)
    ...
    def _increment_search_attempt_count(self):
        try:
            ...
            search_attempt_count = int(search_details_parser_at_active_header[sd_keys.SEARCH_ATTEMPT_COUNT])
            ml.log_event('search try counter at {}, incrementing..'.format(search_attempt_count))
            search_details_parser_at_active_header[sd_keys.SEARCH_ATTEMPT_COUNT] = str(search_attempt_count + 1)
            # self.parsers.search_detail_parser[self.active_header][s_keys.SEARCH_ATTEMPT_COUNT] = str(search_attempt_count + 1)  # TODO delete
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)
    ...
    def _manage_state_updates(self, section_states):
        try:
            ...
            as_keys, uc_keys = self.key_ring.api_state_keyring, self.key_ring.user_config_keyring
            ...
            if _search_queued and not _search_queue_full:
                ...
                if as_keys.RUNNING in _search_status:
                    pass  # search is ongoing, do nothing
                elif as_keys.STOPPED in _search_status:
                    self._update_search_states(as_keys.STOPPED)  # mark search as stopped (finished)
                else:
                    self._update_search_states(as_keys.QUEUED)  # search status unexpected, re-queue this search
                ...
                if filtered_results is not None and filtered_results_count > 0:
                    ...
                    self._update_search_states(as_keys.QUEUED)  # search stopped, no results found, re-queue
            elif _search_concluded:
                pass
            else:
                self._update_search_states(as_keys.QUEUED)
    ...
    def _qbit_add_result(self, result):
        try:
            sd_keys, uc_keys = self.key_ring.search_detail_keyring, self.key_ring.user_config_keyring
            ...
            if results_added > 0:
                ...
                self.parsers.search_details_parser[self.active_header][sd_keys.RESULT_ADDED_COUNT] = str(int(
                    self.parsers.search_details_parser[self.active_header][sd_keys.RESULT_ADDED_COUNT]
                ))
                return
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)
    ...
    def _qbit_get_search_status(self) -> str:
        ...
        try:
            ...
            search_details_parser_with_active_header = self._get_search_details_parser_at_active_header()
            # TODO delete
            # search_id, search_status = self.parsers.search_detail_parser[self.active_header][sd_keys.SEARCH_ID], None
            search_id, search_status = search_details_parser_with_active_header[sd_keys.SEARCH_ID], None
            search_id_valid = self._active_header_search_id_is_valid()
            if search_id_valid:
    ...
    def _reset_search_ids(self):
        ...
        try:
            ap_keys, uc_keys = self._get_api_states_keyring(), self._get_keyring_for_user_config()
            search_details_parser = self.config.parser.parsers.search_details_parser
            for section_header in search_details_parser.sections():
                self.active_header = section_header
                ml.log_event('reset search_id for section: {}'.format(self.active_header))
                self._update_search_states(ap_keys.QUEUED)
                self.pause_on_event(uc_keys.USER)
    ...
    def _result_has_enough_seeds(self, result) -> bool:
        try:
            md_keys, sd_keys = self.key_ring.metadata_keyring, self.key_ring.search_detail_keyring
            # minimum_seeds = int(self.search_parser[self.active_header][search_key.minimum_seeds])  # TODO delete
            minimum_seeds = int(self.parsers.search_details_parser[self.active_header][sd_keys.MIN_SEED_COUNT])
    ...
    def _save_remote_metadata_to_local_results_sorting_by_(self, attribute, filtered_results) -> bool:
        ...
        """
        try:
            ap_keys = self.key_ring.api_state_keyring
            ...
            if not search_id_valid:
                ...
                self._update_search_states(ap_keys.QUEUED)  # wanted to add result but id bad, re-queue search
                return False
    ...
    def _search_id_active(self) -> bool:
        try:
            ...
            search_id = self.parsers[self.active_header][sd_keys.SEARCH_ID]
    ...
    def _active_header_search_id_is_valid(self) -> bool:
        ...
        misc_keys = self.key_ring.misc_keyring  # TODO migrate MiscKeyring() values into other class
        try:
            if search_id is not None:
                if search_id != misc_keys.EMPTY:
                    return True
            return False
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)
    ...
    def _search_has_yielded_the_required_results(self) -> bool:
        ...
        try:
            sd_keys, stop_keys = self.key_ring.search_detail_keyring, self.key_ring.search_stopped_reason_keys
            search_detail_parser = self.parsers.search_details_parser
            attempted_searches = int(search_detail_parser[self.active_header][sd_keys.SEARCH_ATTEMPT_COUNT])
            max_search_attempt_count = int(search_detail_parser[self.active_header][sd_keys.MAX_SEARCH_ATTEMPT_COUNT])
            results_added = int(search_detail_parser[self.active_header][sd_keys.RESULT_ADDED_COUNT])
            results_required = int(search_detail_parser[self.active_header][sd_keys.RESULT_REQUIRED_COUNT])
            if results_added > results_required:
                ...
                self._search_set_end_reason(stop_keys.REQUIRED_RESULT_COUNT_FOUND)  # enough results, concluded
                return True
                ...
                self._search_set_end_reason(stop_keys.TIMED_OUT)  # too many search attempts, conclude
                return True
    ...
    def _search_set_end_reason(self, reason):
        try:
            ...
            self.parsers.search_details_parser[self.active_header][sd_keys.SEARCH_STOPPED_REASON] = reason
    ...
    def _start_search(self):
        try:
            api_keys, misc_keys, search_term = \
                self._get_api_states_keyring(), self._get_keyring_for_misc(), self._parsers_get_search_term()
            search_job, search_status, search_state, search_id, search_count = \
                self._qbit_create_search_job(search_term, 'all', 'all')
            if search_id is not None:
                if search_id is not misc_keys.EMPTY:
                    # TODO potential bug fix? seems to fix bug, delete this line whenever
                    self.active_search_ids[self.active_header] = search_id
            if api_keys.RUNNING in search_state:  # search started successfully
                ...
                self._update_search_states(api_keys.RUNNING)
            elif api_keys.STOPPED in search_status:
                ...
                ml.log_event('search_state is not {} or {}, there was a problem starting the search!'.format(
                    api_keys.RUNNING, api_keys.STOPPED), level=ml.ERROR)
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)
    ...
    def _update_search_states(self, api_state_key):
        ...
        try:
            ps_keys, sd_key, api_keys = \
                self._get_parser_keyring(), self._get_keyring_for_search_details(), self._get_api_states_keyring()
            search_parser_at_active_header, yes, no = \
                self._get_search_details_parser_at_active_header(), ps_keys.YES, ps_keys.NO
            yes, no = ps_keys.YES, ps_keys.NO
            if api_state_key == api_keys.QUEUED:
                search_parser_at_active_header[api_keys.QUEUED] = yes
                search_parser_at_active_header[api_keys.RUNNING] = no
                search_parser_at_active_header[api_keys.STOPPED] = no
                search_parser_at_active_header[api_keys.CONCLUDED] = no
                self.config.parser.parsers.search_details_parser.remove_section(sd_key.SEARCH_ID)
            elif api_state_key == api_keys.RUNNING:
                search_parser_at_active_header[api_keys.QUEUED] = no
                search_parser_at_active_header[api_keys.RUNNING] = yes
                search_parser_at_active_header[api_keys.STOPPED] = no
                search_parser_at_active_header[api_keys.CONCLUDED] = no
                self._increment_search_attempt_count()
            elif api_state_key == api_keys.STOPPED:
                search_parser_at_active_header[api_keys.QUEUED] = no
                search_parser_at_active_header[api_keys.RUNNING] = no
                search_parser_at_active_header[api_keys.STOPPED] = yes
                search_parser_at_active_header[api_keys.CONCLUDED] = no
            elif api_state_key == api_keys.CONCLUDED:
                search_parser_at_active_header[api_keys.QUEUED] = no
                search_parser_at_active_header[api_keys.RUNNING] = no
                search_parser_at_active_header[api_keys.STOPPED] = no
                search_parser_at_active_header[api_keys.CONCLUDED] = yes
            else:
                pass
            # self.config.parser.parsers.search_detail_parser[active_header][sd_key.LAST_WRITE] = str(datetime.now())
            dt_now = str(datetime.now())
            search_parser_at_active_header[sd_key.LAST_WRITE] = dt_now  # TODO what???
        except Exception as e_err:
            ml.log_event(e_err, level=ml.ERROR)
